...
@section('title')
{{__('caption.cms.menu-item.message-menu.edit',['name'=>$message->content??''] )}}
@endsection
<?php $is_RTL = \Config::get('app.locale') == 'ar';  ?>
@section('content')
<div class="row">
	<div class="col-12">
		<h1>{{__('caption.cms.menu-item.message-menu.edit',['name'=>$message->content??''] )}}</h1>
	</div>
</div>
...
<form action="{{route('message.update',$message->id)}}" method="POST">
	...
	<div class="row">  
		...
		<x-textarea
			...
			:initValue="(old('content')??$message->content)??null"
			:rows="4"
			:cols="10"
			placeholder=""
		...
		<x-dropdown-list
			...
			:initValue="(old('created_by_id')??$message->created_by_id)??null"
		...
		<x-dropdown-list
			idName="chat_id"
			:initValue="(old('chat_id')??$message->chat_id)??null"
